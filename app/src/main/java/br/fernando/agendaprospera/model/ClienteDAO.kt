...
//        ENVIANDO DADOS PARA O BANCO
        val values = ContentValues().apply {
            put(Constantes.COLUNA_AGENTE_NOME, agente.nome)
            put(Constantes.COLUNA_AGENTE_LOGIN, agente.login)
            put(Constantes.COLUNA_AGENTE_SENHA, agente.senha)
        }
//        FANEDO O INSERT NO BANCO DE DADOS
        return db.insert(Constantes.TABELA_AGENTE, null, values)
    }
...
//        preparando os dados par atualiacao no banco de dados
        val values = ContentValues().apply {
            put(Constantes.COLUNA_AGENTE_ID, agente.id)
            put(Constantes.COLUNA_AGENTE_NOME, agente.nome)
            put(Constantes.COLUNA_AGENTE_LOGIN, agente.login)
            put(Constantes.COLUNA_AGENTE_SENHA, agente.senha)
        }
//        argumento para comparar identificador
        val selecao = Constantes.COLUNA_AGENTE_ID + " = ?"
...
//        faendo a atualiacao dos dados no banco de dados segundo os argumentos
        return db.update(Constantes.TABELA_AGENTE, values, selecao, selecaoArgumento).toLong()
...
//        SCRIPT PARA REALIAR A CONSULTA NO BANCO DE DADOS
        val selectQuery =
            "SELECT * FROM ${Constantes.TABELA_AGENTE} WHERE ${Constantes.COLUNA_AGENTE_ID} = ? $id"
//        CURSOR PARA BUSCAR O OBJETO NO BANCO DE DADOS
        val cursor = db.rawQuery(selectQuery, null)
        cursor?.moveToFirst()
        agente.id = cursor.getInt(cursor.getColumnIndex(Constantes.COLUNA_AGENTE_ID))
        agente.nome = cursor.getString(cursor.getColumnIndex(Constantes.COLUNA_AGENTE_NOME))
        agente.login = cursor.getString(cursor.getColumnIndex(Constantes.COLUNA_AGENTE_LOGIN))
        agente.senha = cursor.getString(cursor.getColumnIndex(Constantes.COLUNA_AGENTE_SENHA))
        cursor.close()
        return agente
...
//        argumento para consulta
        val selecaoArgumento = "SELECT * FROM ${Constantes.TABELA_AGENTE}"
...
//        argumento para comparacao de identificador
        val selecaoClausula = "${Constantes.COLUNA_AGENTE_ID} = ? $id"
//        argumento passado por parametro
        val argumento = arrayOf(id.toString())
//        removendo objeto do banco de dados
        val removido: Int = db.delete(Constantes.TABELA_AGENTE, selecaoClausula, argumento)
        return removido > 0
    }
